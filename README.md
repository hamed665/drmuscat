...
3. `TAX-C1-PLAN` — final migration boundary plan for vertical/category tables before any DB change.
4. `TAX-C1-DB` — future migration-only implementation for vertical/category tables after plan approval.
5. `OFF-A` — Official Offers admin/data model planning.
6. `ADS-B` / `MON-D` — fuller Ads and paid placement options planning.
7. `ART-B` — Article CMS data model planning.
8. `PLAC-A` — Article placement slot engine planning.
9. `AI-A` — read-only content intelligence planning only after article/offer/placement foundations are approved.
