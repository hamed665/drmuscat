...
const allowedReferenceFiles = new Set([
  'src/config/geo/location-candidate-source-model-contract.ts',
  'src/lib/geo/oman-location-candidate-source-model.ts',
  'src/lib/geo/oman-location-candidate-source-model.test.ts',
...
const forbiddenUsageTokens = [
  'location-candidate-source-model-contract',
  'oman-location-candidate-source-model',
...
function requireTokens(label, source, tokens) {
  ...
  if (missing.length > 0) {
    console.error(`${label} is missing required source model integration guard tokens:`);
...
if (violations.length > 0) {
  console.error('Candidate source model runtime must not be integrated into routes, sitemap, registry, UI, database, or public surfaces yet.');
...
requireTokens('package.json', packageJson, [
  'seo:location-candidate-source-model-integration:validate',
  'check-location-candidate-source-model-integration.mjs',
]);

console.log('Location candidate source model integration guard passed.');
