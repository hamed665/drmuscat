...
function getEventDetail(event: ProviderOnboardingLeadHistoryEvent): string {
  if (event.eventType === "status_changed") {
    ...
    return `${formatLabel(event.oldStatus)} to ${formatLabel(event.newStatus)}`;
  }
  ...
  if (event.eventType === "priority_changed") {
    ...
    return `${formatLabel(event.oldPriority)} to ${formatLabel(event.newPriority)}`;
  }

  if (event.eventType === "note_added") {
    return event.noteText?.trim() || "Admin note saved.";
  }

  return "A lead activity entry was recorded for this lead.";
}
