...
import { buildPublicSearchQueryVariants, dedupePublicSearchItems } from '@/lib/catalog/public-search';
import type {
  PublicCatalogLocale,
...
type PublicSearchResultsPageProps = {
...
const copyByLocale: Record<SupportedLocale, SearchCopy> = {
  en: {
    ...
    centersTitle: 'Centers, hospitals, labs and pharmacies',
  ...
  ar: {
    ...
    centersTitle: 'المراكز والمستشفيات والمختبرات والصيدليات',
...
async function searchPublicCatalogWithVariants(
  query: string,
  options: { limit: number },
): Promise<PublicCatalogQueryResult<PublicCatalogSearchResult> | null> {
  const queryVariants = buildPublicSearchQueryVariants(query);
  if (queryVariants.length === 0) return null;

  const results = await Promise.all(queryVariants.map((queryVariant) => searchPublicCatalog(queryVariant, options)));
  ...
  return {
    ok: true,
    data: {
      centers: dedupePublicSearchItems(results.flatMap((result) => result.data.centers), options.limit),
      doctors: dedupePublicSearchItems(results.flatMap((result) => result.data.doctors), options.limit),
      services: dedupePublicSearchItems(results.flatMap((result) => result.data.services), options.limit),
      areas: dedupePublicSearchItems(results.flatMap((result) => result.data.areas), options.limit),
    },
    emptyReason: null,
    error: null,
  };
...
export async function PublicSearchResultsPage({ locale, country, query }: PublicSearchResultsPageProps) {
  const dir = localeDirection(locale);
  const copy = copyByLocale[locale];
  const result = query.length >= 2 ? await searchPublicCatalogWithVariants(query, { limit: SEARCH_RESULT_LIMIT }) : null;
  ...
  return (
    <main className="home-foundation dm2026-home-page dm-public-search" dir={dir} data-country={country} data-locale={locale}>
      <section className="mx-auto grid w-full max-w-6xl gap-5 px-4 py-8 sm:px-6 lg:px-8 lg:py-12" aria-labelledby="dm-public-search-title">
        <div className="rounded-[2rem] border border-emerald-100/80 bg-gradient-to-br from-white via-emerald-50/40 to-cyan-50/40 p-5 shadow-[0_24px_70px_rgba(15,118,110,0.1)] ring-1 ring-white/80 sm:p-8">
          ...
          <h1 id="dm-public-search-title" className="mt-4 max-w-4xl text-4xl font-semibold leading-[0.98] tracking-[-0.05em] text-slate-950 sm:text-5xl lg:text-6xl">{copy.title}</h1>
        ...
        {!query ? <ResultGroup title={copy.emptyTitle}><p className="text-sm leading-7 text-slate-600">{copy.emptyBody}</p></ResultGroup> : null}
        {result && !result.ok ? <ResultGroup title={copy.errorTitle}><p className="text-sm leading-7 text-slate-600">{copy.errorBody}</p></ResultGroup> : null}
        {result?.ok && !hasResults ? <ResultGroup title={copy.noResults}><p className="text-sm leading-7 text-slate-600">{copy.noResults}</p></ResultGroup> : null}
          ...
          <ResultGroup title={copy.doctorsTitle}>
            <ul className="grid gap-3 md:grid-cols-2" role="list">
              {result.data.doctors.map((doctor: PublicDoctorSummary) => {
                const name = preferredText(locale, doctor.fullNameEn, doctor.fullNameAr) ?? doctor.fullNameEn;
          ...
          <ResultGroup title={copy.centersTitle}>
            <ul className="grid gap-3 md:grid-cols-2" role="list">
              {result.data.centers.map((center: PublicCenterSummary) => {
                ...
                const description = preferredText(locale, center.shortDescriptionEn, center.shortDescriptionAr) ?? preferredText(locale, center.descriptionEn, center.descriptionAr);
                const href = publicCenterDetailRoute(locale, center.defaultCountry, center.slug);
          ...
          <ResultGroup title={copy.servicesTitle}>
            <ul className="grid gap-3 md:grid-cols-2" role="list">
              {result.data.services.map((service: PublicServiceSummary) => {
                const name = preferredText(locale, service.nameEn, service.nameAr) ?? service.nameEn;
                const description = preferredText(locale, service.descriptionEn, service.descriptionAr);
          ...
          <ResultGroup title={copy.areasTitle}>
            <ul className="grid gap-3 md:grid-cols-2" role="list">
              {result.data.areas.map((area: PublicGeoAreaSummary) => {
                const name = preferredText(locale, area.nameEn, area.nameAr) ?? area.nameEn;
