-- IMPORT-PUBLISH-U: atomic pharmacy private publish RPC
-- Scope: one reserved pharmacy center mutation plus terminal persistence in one transaction.
-- The function keeps the entity private: status=draft, is_active=false, no public/index/sitemap state.
...
DECLARE
  ...
  v_allowed_keys constant text[] := ARRAY[
    ...
    'default_locale','default_country','metadata'
  ];
  v_key text;
BEGIN
  ...
  SET
    ...
    default_locale = CASE WHEN p_patch ? 'default_locale' THEN (p_patch->>'default_locale')::public.app_locale ELSE default_locale END,
    default_country = CASE WHEN p_patch ? 'default_country' THEN (p_patch->>'default_country')::public.country_code ELSE default_country END,
...
COMMENT ON FUNCTION public.import_publish_pharmacy_private(
  uuid, uuid, uuid, uuid, uuid, text, jsonb, text
) IS 'Atomic service-role-only private pharmacy mutation with reservation validation, optimistic locking, and terminal audit persistence.';
